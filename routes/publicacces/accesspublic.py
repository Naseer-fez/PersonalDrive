...
from .tookengeneration import verification,verify_token,maplink
...
def Home(userid,filesharing,time,tooken):
    ...
    data=maplink(userid=userid,filepath=filesharing,exptime=time,change=0)
    ...
    filepath,filesize,filetype=filedetails((userid),filesharing)
    if filepath is None:
